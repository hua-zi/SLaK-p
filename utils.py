...
class SmoothedValue(object):
    ...
    def synchronize_between_processes(self):
        ...
        if not is_dist_avail_and_initialized():
            return
        t = paddle.to_tensor([self.count, self.total], dtype='float64', place='gpu')
        dist.barrier()
        dist.all_reduce(t)
        t = t.tolist()
        self.count = int(t[0])
    ...
    def median(self):
        ...
        # print('ddddddddddddddd', d,d.shape)
        # print('sssssssss',self.deque)
...
class MetricLogger(object):
    ...
    def update(self, **kwargs):
        for k, v in kwargs.items():
            ...
            # print('jjjjjjjjjjj',type(self.meters[k]),self.meters[k])
            # print('jjjjjjjj', self.meters[k])
            # print('----------')
    ...
    def __str__(self):
        loss_str = []
        # print('nnnnnnnnnnnnnnn')s
        for name, meter in self.meters.items():
            # print('nnnnnnnnnnn name', name,meter)
            loss_str.append(
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        for obj in iterable:
            ...
            # print('ttttttttttttt', time.time() - end, data_time.count, data_time.deque)
            yield obj
...
def init_distributed_mode(args):
    ...
    paddle.device.set_device(args.gpu)
    ...
    def __call__(self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True):
        ...
        if update_grad:
            if clip_grad is not None:
                ...
                norm = paddle.nn.ClipGradByNorm(parameters, clip_grad)
...
def get_grad_norm_(parameters, norm_type: float = 2.0) -> paddle.Tensor:
    ...
    if norm_type == inf:
        ...
        total_norm = paddle.norm(torch.stack([torch.norm(p.grad.detach(), norm_type) for p in parameters]), norm_type)
    return total_norm
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if args.resume:
        ...
        #             model_ema.ema.set_state_dict(checkpoint['model_ema'])
        #         else:
        #             model_ema.ema.set_state_dict(checkpoint['model'])
