...
def SNIP(net, keep_ratio, train_dataloader, device, masks, args):
    ...
    input_var = paddle.to_tensor(images, place=device)
    target_var = paddle.to_tensor(labels, place=device)
    ...
    for name, weight in net.named_parameters():
        ...
        grads_abs.append(paddle.abs(weight*weight.grad))

    # Gather all scores in a single vector and normalise
    all_scores = paddle.concat([paddle.flatten(x) for x in grads_abs])

    num_params_to_keep = int(len(all_scores) * keep_ratio)
    threshold, _ = paddle.topk(all_scores, num_params_to_keep, sorted=True)
    acceptable_score = threshold[-1]

    layer_wise_sparsities = []
    for g in grads_abs:
        mask = (g > acceptable_score).astype('float32')
...
class CosineDecay(object):
    ...
    def __init__(self, prune_rate, T_max, eta_min=0.005, last_epoch=-1, init_step=0):
        ...
        self.scheduler = paddle.optimizer.lr.CosineAnnealingDecay(learning_rate=prune_rate, T_max=T_max)
        parameters = paddle.create_parameter([1],dtype='float32',default_initializer=paddle.nn.initializer.Constant(value=1.0))
        self.cosine_stepper = optim.SGD(learning_rate=self.scheduler, parameters=paddle.nn.ParameterList([parameters]))
    ...
    def get_dr(self, prune_rate):
        ...
        if self.cosine_stepper.type is 'sgd':
            return self.cosine_stepper._learning_rate
        
        print('kkkkkkkkkkkkkkkk',len(), type(self.cosine_stepper._param_groups[0]),self.cosine_stepper.type)
        for key in self.cosine_stepper._param_groups[0]:
            print('kkkkkkkkkk', key)
        print('kkkkkkkkkkk', self.cosine_stepper._param_groups[0])
        return self.cosine_stepper._param_groups[0]['learning_rate']
...
class Masking(object):
    ...
    def __init__(self, optimizer, train_loader, prune_rate_decay, prune_rate=0.5, prune_mode='magnitude', growth_mode='random', redistribution_mode='momentum', verbose=False, fp16=False, args=False):
        ...
        self.args = args
        # self.device = paddle.device(args.device)
    ...
    def add_module(self, module):
        ...
        for name, tensor in module.named_parameters():
            if len(tensor.shape) == 2 or len(tensor.shape) == 4:
                if self.args.only_L:
                    if 'large_kernel.LoRA' in name:
                        self.names.append(name)
                        self.masks[name] = paddle.to_tensor(paddle.zeros_like(tensor, dtype='float32'), stop_gradient=True,place=self.device)
                else: # √
                    self.names.append(name)
                    self.masks[name] = paddle.to_tensor(paddle.zeros_like(tensor, dtype='float32'), stop_gradient=True,place=self.device)
    ...
    def init(self, mode='snip', density=0.05, erk_power_scale=1.0):
        ...
        if mode == 'uniform':
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    # self.masks[name][:] = (paddle.rand(weight.shape) < density).float().data.to(self.device)
                    self.masks[name][:] = paddle.to_tensor((paddle.rand(weight.shape) < density),dtype='float32',place=self.device)
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    self.masks[name][:] = paddle.to_tensor((weight != 0.0),dtype='float32',place=self.device)
                    self.baseline_nonzero += weight.numel()*density
            ...
            for sparsity_, name in zip(layer_wise_sparsities, self.masks):
                ...
                #     self.device)
                self.masks[name][:] = paddle.to_tensor((paddle.rand(self.masks[name].shape) < (1 - sparsity_)),dtype='float32',place=
                    self.device)
            ...
            for name, mask in self.masks.items():
                ...
                self.masks[name][:] = paddle.to_tensor((paddle.rand(mask.shape) < density_dict[name]),dtype='float',place=self.device)
        ...
        for name, weight in self.masks.items():
            ...
            sparse_weight_num = (weight != 0).sum().astype('int32').item()
            ...
            print(f'Density of layer {name} with tensor {weight.shape} is {layer_density}')
    ...
    def apply_mask(self):
        ...
        # for module in self.modules:
        #     for name, tensor in module.named_parameters():
        #         print('hhhhhhhhh',name,type(tensor),tensor.shape)
        #         if name in self.masks:
        #             if not self.half:
        #                 # tensor.data = tensor.data*self.masks[name]
        #                 # tensor.set_value(tensor*self.masks[name])
        #                 tensor = tensor*self.masks[name]
        #                 # if 'momentum_buffer' in self.optimizer.state_dict()[tensor]:
        #                 if 'momentum_buffer' in self.optimizer.state_dict()[tensor]:
        #                     self.optimizer.state_dict()[tensor]['momentum_buffer'] = self.optimizer.state_dict()[tensor]['momentum_buffer']*self.masks[name]
        #             else:
        #                 # tensor.data = tensor.data*self.masks[name].half()
        #                 tensor.set_value(tensor*self.masks[name].astype('float16'))
        #                 if name in self.name_to_32bit:
        #                     tensor2 = self.name_to_32bit[name]
        #                     # tensor2.data = tensor2.data*self.masks[name]
        #                     tensor2.set_value(tensor2*self.masks[name])
    ...
    def get_momentum_for_weight(self, weight):
        if 'exp_avg' in self.optimizer.state_dict()[weight]:
            adam_m1 = self.optimizer.state_dict()[weight]['exp_avg']
            adam_m2 = self.optimizer.state_dict()[weight]['exp_avg_sq']
            grad = adam_m1/(paddle.sqrt(adam_m2) + 1e-08)
        elif 'momentum_buffer' in self.optimizer.state_dict()[weight]:
            grad = self.optimizer.state_dict()[weight]['momentum_buffer']
    ...
    def fired_masks_update(self):
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                ...
                self.fired_masks[name] = self.masks[name].astype('uint8') | self.fired_masks[name].astype('uint8')
    ...
    def synchronism_masks(self):
        ...
        for name in self.masks.keys():
            ...
            paddle.distributed.broadcast(self.masks[name], src=0)
