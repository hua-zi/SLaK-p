...
class RandomResizedCropAndInterpolation:
    ...
    def get_params(img, scale, ratio):
        ...
        Returns:
            tuple: params (i, j, h, w) to be passed to ``crop`` for a random
                sized crop.
        """
        # print('iiiiiiiiiiiiwork/SLaK/timm/data/transforms.py', type(img))
    ...
    def __call__(self, img):
        ...
        # interpolation = interp_mode_to_str(interpolation)
        # print('-------',interpolation)
        # return F.resized_crop(img, i, j, h, w, self.size, interpolation)
        # print('-hhhhhhhhhh',type(img),type(F.crop(img, i, j, h, w)))
        # c = F.crop(img, i, j, h, w)
        # print('iiiiiiiiiiiiiwork/SLaK/timm/data/transforms.py', self.size, type(self.size))
        # return F.resize(c, self.size, interpolation)
    ...
    def __repr__(self):
        if isinstance(self.interpolation, (tuple, list)):
            ...
            interpolate_str = ' '.join([x for x in self.interpolation])
        else:
            # interpolate_str = interp_mode_to_str(self.interpolation)
            interpolate_str = self.interpolation
        format_string = self.__class__.__name__ + '(size={0}'.format(self.size)
        # format_string = self.__class__.__name__ + '(shape={0}'.format(self.shape)
